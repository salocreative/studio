...
- **Product cards** (`app/actions/product-cards.ts`, `lib/product-cards/`): repeatable products at `/product-cards`, published to products.salo.uk. Admins, designers, and managers can add and edit cards, including covers and examples. The public site calls `get_product_card`, `list_product_cards` and `log_product_card_view` with the anon key and has no table access.
